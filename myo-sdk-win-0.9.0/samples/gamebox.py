...
from Sudoku import Small_Square, Large_Square, Board
import pygame, sys
...
class Camera(object):
    ...
    def draw(self, thing, *args):
        ...
        elif isinstance(thing, Small_Square.SmallSquare):
            self.draw(thing.image)
            self.draw(thing.cover)
            self.draw(thing.text_display)
        elif isinstance(thing, Large_Square.LargeSquare):
            self.draw(thing.large)
        elif isinstance(thing, list):
            for item in thing:
                self.draw(item)
        elif isinstance(thing, Board.Board):
            self.draw(thing.Squares)
        else:
            print(type(thing))
